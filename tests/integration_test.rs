...
#[test]
fn test_simulate_mode_flexible_match_allow() {
    let output = ccjanus_bin()
        .args([
            "simulate",
            "--flexible-match",
...
#[test]
fn test_simulate_mode_flexible_match_deny() {
    let output = ccjanus_bin()
        .args([
            "simulate",
            "--flexible-match",
...
#[test]
fn test_simulate_mode_without_flexible_match_fallthrough() {
    let output = ccjanus_bin()
        .args([
            "simulate",
            "--command",
...
#[test]
fn test_hook_mode_flexible_match_from_settings() {
    let tmp = tempfile::TempDir::new().unwrap();
    let settings_path = tmp.path().join("settings.json");
    std::fs::write(
        &settings_path,
        r#"{"permissions":{"allow":["Bash(uv run ruff format *)"],"flexible_match":true}}"#,
    )
    .unwrap();
    ...
    stdin
        .write_all(
            br#"{"tool_name":"Bash","tool_input":{"command":"uv run --group dev ruff format chatbot-agent/"}}"#,
        )
        .unwrap();
    drop(child.stdin.take());
